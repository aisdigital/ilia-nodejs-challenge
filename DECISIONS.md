...
- O Swagger (`@nestjs/swagger` + `swagger-ui-express`) foi disponibilizado por import dinâmico nos arquivos `main.ts` de cada serviço.
- Motivo: tornar a dependência opcional para setups mínimos (dev/CI) e não provocar falhas quando a dependência não estiver instalada. A rota da documentação é `/api` (somente em ambiente não-`test`).
