...
export class Logger {
  ...
  private serviceName: string = 'ms-wallet';
  ...
  private createLogger(): winston.Logger {
    ...
    if (isProduction || process.env.ENABLE_FILE_LOGS === 'true') {
      ...
      transports.push(
        new DailyRotateFile({
          filename: path.join(logsDir, 'wallet-%DATE%.log'),
      ...
      transports.push(
        new DailyRotateFile({
          filename: path.join(logsDir, 'wallet-error-%DATE%.log'),
          datePattern: 'YYYY-MM-DD',
          maxSize: '20m',
          maxFiles: '30d',
          format: structuredFormat,
      ...
      transports.push(
        new DailyRotateFile({
          filename: path.join(logsDir, 'wallet-audit-%DATE%.log'),
          datePattern: 'YYYY-MM-DD',
          maxSize: '20m',
