...
import { WalletService } from '../services/queue-simulated.service';
...
export class UserController {
  ...
  @Get(':id')
  @Authorize(['BASIC', 'EXCLUSIVE', 'PRIME'])
  ...
  @Post('/api/internal/wallets/:id/:balance')
  @Authorize(['BASIC', 'EXCLUSIVE', 'PRIME'])
  @ApiOperation({ summary: 'Show user details by username' })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async showByUsername(@Param('id') id: number, @Param('balance') balance: number,): Promise<void> {
    const balanceDeposit = await this.walletService.deposit(id, { balance });
